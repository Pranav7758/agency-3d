...
const initPlanet = (canvas: HTMLCanvasElement): {
  ...
  const noopResult = { scene: null, renderer: null, timeline: null, tickerFunc: () => {}, resizeFunc: () => {}, observer: null };

  // Test WebGL availability before attempting to initialize
  const testCanvas = document.createElement("canvas");
  const testCtx = testCanvas.getContext("webgl") ?? testCanvas.getContext("experimental-webgl");
  if (!testCtx) return noopResult;
